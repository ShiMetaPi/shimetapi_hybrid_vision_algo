...
#include <hv_algo/denoise/khodamoradi_denoiser.h>
...
int main(int argc, char *argv[]) {
    ...
    // Create a Khodamoradi Denoiser instance
    // Adjust parameters as needed for your application
    Shimeta::Algorithm::Denoise::KhodamoradiDenoiser khodamoradi_denoiser(camera_width, camera_height, 2000, 2); // 示例参数，可根据实际需求调整
    ...
    Metavision::Window window("Metavision SDK Khodamoradi Denoising", camera_width, camera_height,
    ...
    cam.cd().add_callback([&](const Metavision::EventCD *begin, const Metavision::EventCD *end) {
        ...
        std::vector<Metavision::EventCD> denoised_events = khodamoradi_denoiser.process_events(input_events);

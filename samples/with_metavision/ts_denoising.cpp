...
#include <hv_algo/denoise/timesurface_denoisor.h>
...
int main(int argc, char *argv[]) {
    ...
    // 创建 Time Surface Denoisor 实例，参数可根据实际需求调整
    Shimeta::Algorithm::Denoise::TimeSurfaceDenoisor ts_filter(camera_width, camera_height, 2000, 2);
    ...
    Metavision::Window window("Metavision SDK TS Denoising", camera_width, camera_height,
    ...
    cam.cd().add_callback([&](const Metavision::EventCD *begin, const Metavision::EventCD *end) {
        ...
        std::vector<Metavision::EventCD> denoised_events = ts_filter.process_events(input_events);

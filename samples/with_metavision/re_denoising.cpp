...
#include <hv_algo/denoise/reclusive_event_denoisor.h>
...
int main(int argc, char *argv[]) {
    ...
    // 创建RED去噪器实例，参数可根据实际需求调整
    Shimeta::Algorithm::Denoise::ReclusiveEventDenoisor red_filter(camera_width, camera_height, 2000, 2);
    ...
    Metavision::Window window("Metavision SDK RED Denoising", camera_width, camera_height,
    ...
    cam.cd().add_callback([&](const Metavision::EventCD *begin, const Metavision::EventCD *end) {
        ...
        std::vector<Metavision::EventCD> denoised_events = red_filter.process_events(input_events);
